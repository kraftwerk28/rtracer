use crate::{camera::Camera, geometry::Face, vector::Vec3 as Vec3_};
use std::{
    env::current_dir,
    fs::File,
...
pub fn parse(filename: String) -> WavefrontObj {
    ...
    let mut result = WavefrontObj::init();
        ...
        match tokens.nth(0).unwrap() {
            ...
            "f" => {
                let face: Vec<usize> = tokens
                    .take(3)
                    .map(|s| {
                        let f64_chars: String =
                            s.chars().take_while(|c| c.is_numeric()).collect();
                        f64_chars.parse().unwrap()
                    })
                    .collect();
                if face.len() < 3 {
                    println!("Bad .obj format! Aborting.");
                    std::process::exit(1)
                }
